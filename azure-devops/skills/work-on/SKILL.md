...
2. **Ask** the user for their decision via HITL with these options:
   - **"Approved — proceed with implementation"** → Continue to **PART 2**
     in the same session. Record the approval as `APPROVED` in the decision
     log (`decisions.md`) — never as a new or edited BOT-PLAN marker. The
     already-posted plan comment's marker remains the sole, latest BOT-PLAN
     source.

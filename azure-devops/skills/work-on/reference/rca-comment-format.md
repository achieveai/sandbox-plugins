...
the work item's comment thread (an **Azure DevOps work item** comment) when the
work item is a **Bug**. It extends the standard
...
will proceed on the next run of the work-on command (`/work-on <id>`)
regardless of further feedback.*
```

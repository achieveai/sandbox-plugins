...
Work this directly with ordinary tools — no external debugging skill is
delegated to. Given:
...
1. Understand the reported problem — read the description, repro steps, and
   any linked context until the expected-vs-actual behavior is clear.
2. Create repeatable reproduction steps — a minimal command or test case that
   triggers the bug on demand.
3. Execute the repro and collect JSONL logs — run it, capturing structured
   logs (add temporary `console.log`/`logger` JSONL emission around the
   suspected path if none exists yet).
4. Query the logs with DuckDB (`duckdb -c "SELECT ... FROM read_json_auto('...')"`)
   to trace the execution flow and pinpoint where behavior diverges.
5. If the evidence is insufficient, add more targeted logging and re-run —
   repeat until the failure point is unambiguous.
...
- Add JSONL logging directly (Edit/Write) around the suspected code path first
- Then repeat steps 3-5 above for the actual debugging

---

## Stage B — Formulate Root Cause Analysis

Continue directly — confirm root cause with log evidence (step 6 of the
methodology above).

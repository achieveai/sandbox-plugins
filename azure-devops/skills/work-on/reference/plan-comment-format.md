...
item's comment thread (an **Azure DevOps work item** comment). The format is
both human-readable (Markdown) and machine-parseable (HTML comment markers).
...
When re-entering the work-on command (Part 2) — `/work-on <id>` — the bot must
find and parse the latest plan:
...
When the user re-runs the work-on command (`/work-on <id>`) and there are zero
human comments after the latest bot plan comment, the agent MUST still present
the plan summary and ask the user via HITL before proceeding. The agent never
assumes that a re-run alone signals approval — explicit confirmation through
the HITL checkpoint is always required.

...
3. Before the first Azure DevOps CLI call in a session, run `sandbox-auth:azure-devops` (pass the target org if known). If a CLI call fails with `E_AUTH`, run it once and retry. Never ask the user to supply a PAT, token, or credential file.
...
{ "workItemType": "<Bug|Task|User Story>", "title": "<confirmed title>", "description": "<confirmed description>" }
ADOJSON
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/ado-cli.js" createPullRequest --structured <<'ADOJSON'
{ "repository": "<repository name>", "sourceRefName": "refs/heads/<current-branch>", "targetRefName": "refs/heads/<base-branch>", "title": "<title>", "description": "<composed PR description>" }
ADOJSON
```

### Step 2.4: Link Work Item to PR

Use `createLink` to link the work item from Phase 1 to the PR:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/ado-cli.js" createLink --structured <<'ADOJSON'
{ "sourceId": <work_item_id>, "targetId": "PR#<pr_id>", "linkType": "System.LinkTypes.Related", "repository": "<repository name>" }
ADOJSON
```
...
(it will fix issues and push without asking — see `azure-devops:babysit-pr`).
- Say **no** to stop here.

If the user declines, the skill ends. If the user chooses babysit, load and
execute `azure-devops:babysit-pr` with the PR number.

For interactive tending, delegate to the `azure-devops:pr-tender` agent. Pass:
...
Use the `azure-devops:mentions` skill before composing any PR description, comment,
...
- Use git/bash for local operations; use the bundled `ado-cli.js` for all ADO operations

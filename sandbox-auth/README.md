...
| `microsoft-graph` | `/sandbox-auth:microsoft-graph [scopes]` | Set up Microsoft Graph (M365) auth FIRST, then follow its per-domain **recipes** under `reference/` (mail incl. focused inbox & shared mailboxes; calendar & shared calendars; Teams/channel/**meeting chat**; **meeting transcripts, recordings, attendance**; OneDrive/SharePoint; unified search/people/directory). |

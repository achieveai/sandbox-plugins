...
`reference/` — read the one for your domain instead of guessing endpoints).
...
|---|---|---|
| Read/search mail, folders, **focused inbox**, **shared mailboxes**, send | `reference/mail.md` | `/me/messages`, `/me/mailFolders`, `/users/{upn}/messages` |
| Calendar, **shared calendars**, free/busy, find times | `reference/calendar.md` | `/me/calendarView`, `/me/events`, `/users/{upn}/calendar` |
| Teams, channels, **channel messages**, 1:1/group **chats**, **meeting chat** | `reference/teams-chat.md` | `/me/joinedTeams`, `/teams/{id}/channels/{id}/messages`, `/me/chats`, `/chats/{threadId}/messages` |
| **Meeting transcripts, recordings, attendance** | `reference/meetings.md` | `/me/onlineMeetings?$filter=JoinWebUrl eq …` → `/transcripts`, `/recordings`, `/attendanceReports` |
| OneDrive files, **SharePoint** sites/libraries, file download | `reference/files-sharepoint.md` | `/me/drive/...`, `/sites/...`, driveItem `content` |
| Unified **search** (KQL), people, directory, presence | `reference/search-directory.md` | `POST /search/query`, `/me/people`, `/users`, `/me/presence` |
...
1. **Meetings: you CANNOT list `/me/onlineMeetings`.** A bare `GET /me/onlineMeetings` returns
   ...
   `GET /me/onlineMeetings?$filter=JoinWebUrl eq '<joinUrl>'`. See `reference/meetings.md`.

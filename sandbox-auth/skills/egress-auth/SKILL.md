...
name: egress-auth
description: >-
  Core sandbox egress authentication skill — the shared engine that github /
  azure-devops / microsoft-graph / connect reference and use. Authentication is a
  REQUIRED FIRST STEP for any outbound call: the proxy blocks unauthenticated egress.
  Use whenever an outbound call (git clone/fetch/push, gh, curl, wget, npm, pip, or an
  HTTP client) to github.com, api.github.com, a package registry, or any external host
  returns HTTP 511 (header `x-sandbox-proxy-status: auth_pending` or body
  `{"status":"auth_pending",...}`), HTTP 403 `{"error":"denied",...}`, or when a network
  command hangs, fails TLS, or behaves oddly behind the egress proxy. Also use
  PROACTIVELY before token-unaware tools (git/gh/npm/pip) to warm the auth cache. Owns
  the wire contract, probe/poll engine, backoff polling, human-in-the-loop login relay,
  transparent server-side token injection, and the warm-then-run pattern. Other skills
  delegate the handshake here.
allowed-tools:
  - Read
  - Bash
---
